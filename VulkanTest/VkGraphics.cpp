...
void createDescriptorSetLayout(_In_ VkDevice device, 
    ...
    // add sampler layout binding fo rtextures
                            ...
                            _Out_ VkPipeline* graphicsPipeline)
{
    ...
    // define the viewport, subregion onto which image will actually be drawn 
    VkViewport viewport{};
    viewport.x = 0.0f;
    viewport.y = 0.0f;
    ...
    // define the scissor, subregion of the screen outside of which pixels are discarded
    VkRect2D scissor{};
    scissor.offset = { 0, 0 };
    ...
    // depth clamp: discard fragments beyond near and far planes
    // setting to VK_TRUE also requires enabling GPU extension
    rasterizer.depthClampEnable = VK_FALSE;

    // rasterizerDiscardEnable: setting this to VK_TRUE disables output to the framebuffer!
    rasterizer.rasterizerDiscardEnable = VK_FALSE;

    // polygon fill mode,  VK_POLYGON_MODE_FILL will actually fill the polygons,'
    // as opposed to just drawing the lines or a single point
    ...
    // multisampling: works like AA, but only edges so its faster 
    // disabled for now
...
void createCommandBuffers(_In_ VkDevice device,
    ...
    _Out_ std::vector<VkDescriptorSet>& descriptorSets, 
...

{
    ...
    for (size_t i = 0; i < commandBuffers.size(); i++) {
        ...
        // vkCmdDraw(commandBuffers[i], numVertices, 1, 0, 0);
